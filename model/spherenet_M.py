...
class emb(torch.nn.Module):
    def __init__(self, num_spherical, num_radial,edge_feat, cutoff, envelope_exponent): #######
        super(emb, self).__init__()
        self.dist_emb = dist_emb(num_radial-edge_feat, cutoff, envelope_exponent)
    ...
    def forward(self, dist, angle, torsion, idx_kj):
        ...
        #print('dist_emb :',dist_emb.size()) #([658, 6])
        #print('angle_emb :', angle_emb.size()) # ([10700, 18])
        #print('torsion_emb :', torsion_emb.size())# ([10700, 54])
...
class init(torch.nn.Module):
    def __init__(self, num_radial, hidden_channels, act=swish, use_node_features=True):
        ...
        if self.use_node_features:
            self.emb = Embedding(32, hidden_channels).to(device) ####### embedding dict size: atom type number
        else: # option to use no node features and a learned embedding vector for each node instead
            self.node_embedding = nn.Parameter(torch.empty((hidden_channels,))).to(device)
            nn.init.normal_(self.node_embedding)
        self.lin_rbf_0 = Linear(num_radial, hidden_channels).to(device)
        self.lin = Linear(3 * hidden_channels, hidden_channels).to(device)
        self.lin_rbf_1 = nn.Linear(num_radial, hidden_channels, bias=False).to(device)
        self.reset_parameters()
    ...
    def forward(self, x, emb, i, j):
        rbf,_,_ = emb #### distance embedding
        if self.use_node_features:
            print('x',x.device)
            x = self.emb(x)
            #print('x',x.size()) [42,40]
        else:
        ...
        #print('e1:',e1.size())  #([112, 40])
        #print('e2:',e2.size())  #([112, 40])
...
class update_e(torch.nn.Module):
    def __init__(self, hidden_channels, int_emb_size, basis_emb_size_dist, basis_emb_size_angle, basis_emb_size_torsion, num_spherical, num_radial,
        ...
        self.act = act
        self.lin_rbf1 = nn.Linear(num_radial, basis_emb_size_dist, bias=False).to(device)
        self.lin_rbf2 = nn.Linear(basis_emb_size_dist, hidden_channels, bias=False).to(device)
        self.lin_sbf1 = nn.Linear(num_spherical * num_radial, basis_emb_size_angle, bias=False).to(device)
        self.lin_sbf2 = nn.Linear(basis_emb_size_angle, int_emb_size, bias=False).to(device)
        self.lin_t1 = nn.Linear(num_spherical * num_spherical * num_radial, basis_emb_size_torsion, bias=False).to(device)
        self.lin_t2 = nn.Linear(basis_emb_size_torsion, int_emb_size, bias=False).to(device)
        self.lin_rbf = nn.Linear(num_radial, hidden_channels, bias=False).to(device)

        self.lin_kj = nn.Linear(hidden_channels, hidden_channels).to(device)
        self.lin_ji = nn.Linear(hidden_channels, hidden_channels).to(device)

        self.lin_down = nn.Linear(hidden_channels, int_emb_size, bias=False).to(device)
        self.lin_up = nn.Linear(int_emb_size, hidden_channels, bias=False).to(device)

        self.layers_before_skip = torch.nn.ModuleList([
            ResidualLayer(hidden_channels, act).to(device)
            for _ in range(num_before_skip)
        ])
        self.lin = nn.Linear(hidden_channels, hidden_channels).to(device)
    ...
    def forward(self, x, emb, idx_kj, idx_ji):
        rbf0, sbf, t = emb # load embed dist, angle, torsion
        x1,_ = x #load embed h
...
class update_v(torch.nn.Module):
    def __init__(self, hidden_channels, out_emb_channels, out_channels, num_output_layers, act, output_init):
        ...
        self.lin_up = nn.Linear(hidden_channels, out_emb_channels, bias=True).to(device)
        self.lins = torch.nn.ModuleList()
        for _ in range(num_output_layers):
            self.lins.append(nn.Linear(out_emb_channels, out_emb_channels).to(device))
        self.lin = nn.Linear(out_emb_channels, out_channels, bias=False).to(device)
    ...
    def forward(self, e, i):
        ...
        for lin in self.lins: # 4 layers 256*256
            v = self.act(lin(v))
        v = self.lin(v)
        #print('v_update:',v.size())
        return v
...
class SphereNet_M(torch.nn.Module):
    r"""
    ...
    def __init__(
        ...
        num_spherical=7, num_radial=6, envelope_exponent=5,edge_feat=3,
        num_before_skip=1, num_after_skip=2, num_output_layers=3,
        act=swish, output_init='GlorotOrthogonal', use_node_features=True):
        super(SphereNet_M, self).__init__()
        ...
        self.emb = emb(num_spherical, num_radial,edge_feat, self.cutoff, envelope_exponent)
        self.emb.to(device)
    ...
    def forward(self, z, pos, batch, edge_index,edge_attr):
        ...
        d_emb,a_emb,t_emb = self.emb(dist, angle, torsion, idx_kj,)
        print('d_emb',d_emb.device,d_emb.size())
        print('a_emb', a_emb.device,a_emb.size())
        print('t_emb', t_emb.device,t_emb.size())

        d_emb = torch.cat([d_emb.to(device), edge_attr], 1)
        emb=[d_emb,a_emb,t_emb]
        #print(d_emb)
        ...
        print('edge_attr: ', e[0].size(),e[1].size()) #[2*tensor([edge_index_size, hidden_dim=40])]
        print('h\': ', v.size()) #torch.Size([atom_number, 1])
        print('u: ', u.size()) #torch.Size([batch_size, 1])
