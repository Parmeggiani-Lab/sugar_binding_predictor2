...
class dist_emb(torch.nn.Module):
    def __init__(self, num_radial, cutoff=5.0, envelope_exponent=5):
        ...
        #self.freq = torch.nn.Parameter(torch.tensor(num_radial, device=device,dtype=torch.float))
    ...
    def reset_parameters(self):
        self.freq.data = torch.arange(1, self.freq.numel() + 1).float().mul_(PI).to(device)

    def forward(self, dist):
        dist = dist.unsqueeze(-1) / self.cutoff
        dist=dist.to(device)
        print('dist',dist.device)
        print('freq',self.freq,self.freq.device)
