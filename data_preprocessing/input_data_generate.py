...
loc_ori = loc
loc = loc + '/functional_group/'


try:
    print(pass_file_list)
    file_list=pass_file_list
except NameError:
    file_list = []
    for f in os.listdir(loc):
        if re.search('(.*)_elicit\.pdb', f):
            file_list.append(f)
    count_file = len(file_list)
    print(str(count_file) + ' interacting files will process to input of DL\n')
    # print(all_file)

if os.path.exists(loc_ori + out_affix) == False:
    os.mkdir(loc_ori + out_affix)

loc_out = loc_ori + out_affix
...
#for i in aa_keep_atom_dict:
#    current_resi_atom_name_list.extend([i+'_'+j for j in aa_keep_atom_dict[i].split('+')])
#current_atomtype_list=[ atom_type_dict[i] for i in current_resi_atom_name_list]
...
#for i,type in enumerate(unq_atomtype_list):
#    atomtype2id[type]=i+1
...
max_len_seq=0
len_seq_record=[]
for pdb in file_list[:]:#['6HD9_1_poly_56_elicit.pdb']:#['2r_sort_109_W2B_OMJCBMTW_BasicBeNTF2_designed_0000_5_0001_0005_elicit__2r_sort_111_W2A_ZDWJPIHU_BasicBeNTF2_designed_0000_2_0001_0119_elicit.pdb']:#
    print(file_list.index(pdb),pdb)
    ...
    atom_coord_list=np.array([[i[31:38].split()[0],i[39:45].split()[0],i[47:56].split()[0]] for i in atom_line])##################################################
...
############# edge info
    try:
        CH_pi_info= CH_pi_info_dict[pdb[:-11]]
    except KeyError:
        CH_pi_info=[]
    print(CH_pi_info)
    ...
    for i in CH_pi_info:
        ring_id=i.split('_')[1]
        if i.split('_')[1]=='TRB':
            ring_atom_id_list = [atom_resi_id_list.index('TRP' + '_' + str(int(i.split('_')[0])-0) + '_' + j) + 1 for j in
                                 aromatic_ring_atom_dict[ring_id]]
        else:
            ring_atom_id_list=[atom_resi_id_list.index(i.split('_')[1]+'_'+str(int(i.split('_')[0])-0)+'_'+j)+1 for j in aromatic_ring_atom_dict[ring_id]]
    ...
    aa_contact_list=[]###################
    edge_select_list=[]
    edge_select = torch.zeros(batch_size, len(atomtype_id_list), len(atomtype_id_list)).to(torch.int32) ##########################
    ...
    for i in range(len(pairwise_matrix[0])):
        ...
        if math.dist(atom_coord_list[pairwise_matrix[0][i]],atom_coord_list[pairwise_matrix[1][i]]) <3.5 and bond_list[i]==0 and residue_id_list[pairwise_matrix[0][i]]!=residue_id_list[pairwise_matrix[1][i]] and polartype_list[pairwise_matrix[0][i]]==polartype_list[pairwise_matrix[1][i]]  :
            contact_list.append(1)
            #print(int(pairwise_matrix[0][i])+1,int(pairwise_matrix[1][i])+1)
            if (residue_id_list[pairwise_matrix[0][i]].split('_')[0] not in current_sugar_list) and (residue_id_list[pairwise_matrix[1][i]].split('_')[0] not in current_sugar_list):
        ...
        if atompos_list[pairwise_matrix[0][i]] != atompos_list[pairwise_matrix[1][i]] and math.dist(atom_coord_list[pairwise_matrix[0][i]],atom_coord_list[pairwise_matrix[1][i]]) <3.5:
    ...
    print('polar-polar/inpolar-inpolar non_covalent contact between amino acid within 3.5 A: ', sum(aa_contact_list) / 2)
    ...
    if len(piece_list)<=10000:
        with open(loc_out+pdb[:-4]+'_'+'info.txt', 'w') as out_file:
...
print('\n\tlongest piece_seq with 2 <S>:',max_len_seq)
print(len_seq_record)
